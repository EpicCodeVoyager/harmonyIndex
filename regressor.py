...
def remove_highly_correlated_columns(x, thr):
    corr_matrix = x.corr().abs()
    up_tri = corr_matrix.where(np.triu(np.ones(corr_matrix.shape), k=1).astype(np.bool))

    to_drop = [column for column in up_tri.columns if any(up_tri[column] > thr)]
...
# Function to print summary of the model (if disp flag true) and return fit object
def sm_fit(x, y, disp=False):
